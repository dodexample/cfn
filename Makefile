...
check-templates:
	for tpl in $${tpl:-alb asg asg2 env} ; do \
...
cook-parameters:
	...
	cp asg2-parameters.json cooked-asg2-parameters.json && \
	...
	sed -i "" -e "s/SUBNETS/$${subnetids}/g" -e "s/VPCID/$${vpcid}/g"  -e 's/[[:space:]]\{1,\}$$//g' cooked-asg-parameters.json cooked-alb-parameters.json cooked-asg2-parameters.json

apply-templates:
	for tpl in $${tpl:-env alb asg asg2} ; do \
